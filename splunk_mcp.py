...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("splunk_mcp.log")],
)
...
async def custom_swagger_ui_html():
    return get_swagger_ui_html(
        openapi_url="/openapi.json", title=f"{mcp.name} - Swagger UI"
    )


@app.get("/redoc", include_in_schema=False)
async def redoc_html():
    return get_redoc_html(openapi_url="/openapi.json", title=f"{mcp.name} - ReDoc")
        ...
        "paths": {
            ...
            "/messages": {
                "get": {
                    ...
                    "tags": ["MCP Core"],
                    "responses": {"200": {"description": "Message endpoint ready"}},
                }
            },
            "/execute": {
                "post": {
...
def get_splunk_connection() -> splunklib.client.Service:
    ...
    try:
        if SPLUNK_TOKEN:
            logger.debug(
                f"🔌 Connecting to Splunk at {SPLUNK_SCHEME}://{SPLUNK_HOST}:{SPLUNK_PORT} using token authentication"
            )
            ...
            logger.debug(
                f"🔌 Connecting to Splunk at {SPLUNK_SCHEME}://{SPLUNK_HOST}:{SPLUNK_PORT} as {username}"
            )
            service = splunklib.client.connect(
                host=SPLUNK_HOST,
                port=SPLUNK_PORT,
...
async def search_splunk(
    ...
    if not (
        stripped_query.startswith("|") or stripped_query.lower().startswith("search")
    ):
...
async def current_user():  # Return type hint removed - causes schema errors in some MCP clients (may be fixed in FastMCP 3.x)
    ...
    try:
        ...
        try:
            ...
            current_context_resp = service.get(
            ...
            if "entry" in current_context_obj and len(current_context_obj["entry"]) > 0:
                context_username = current_context_obj["entry"][0]["content"].get(
                    "username"
                )
                if context_username:
                    current_username = context_username
                    logger.debug(
                        f"Using username from current-context: {current_username}"
                    )
        except Exception as context_error:
            logger.warning(
                f"⚠️ Could not get username from current-context: {str(context_error)}"
            )
        ...
        try:
            ...
            if hasattr(current_user, "content") and isinstance(
                current_user.content, dict
            ):
                user_info = {
                    ...
                    "default_app": current_user.content.get("defaultApp", "search")
                    or "search",
            ...
            logger.info(
                f"✅ Successfully retrieved current user information: {current_user.name}"
            )
            return user_info
        ...
        for user in service.users:
            try:
                if hasattr(user, "content"):
                    ...
                    users.append(user_info)
                    logger.warning(
                        f"⚠️ User {user.name} has no content, using default values"
                    )
            except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if "entry" in stats_data and len(stats_data["entry"]) > 0:
                ...
                logger.debug(
                    f"✅ Retrieved stats for {len(collection_stats)} KV store collections"
                )
        except Exception as e:
        ...
        try:
            for entry in service.kvstore:
                try:
                    ...
                    fieldsList = [
                        f.replace("field.", "")
                        for f in entry["content"]
                        if f.startswith("field.")
                    ]
                    accelFields = [
                        f.replace("accelerated_field.", "")
                        for f in entry["content"]
                    ...
                    collection_data = {
                        ...
                        "record_count": collection_stats.get(
                            f"{app_name}.{collection_name}", 0
                        ),
                    }
                    collections.append(collection_data)
                    collections_found += 1
                    logger.debug(
                        f"✅ Added collection: {collection_name} from app: {app_name}"
                    )
                except Exception as e:
    ...
    try:
        ...
        response = {
            ...
            "metadata": {
                ...
                "total_sourcetypes": sum(
                    len(st) for st in sourcetypes_by_index.values()
                ),
...
async def list_tools():  # Return type hint removed - causes schema errors in some MCP clients (may be fixed in FastMCP 3.x)
    ...
    try:
        ...
        elif hasattr(mcp, "registered_tools") and isinstance(
            mcp.registered_tools, dict
        ):
            # Access through registered_tools attribute
            for name, tool_info in mcp.registered_tools.items():
                try:
                    description = (
                        tool_info.get("description", None)
                        or getattr(tool_info, "description", None)
                        or "No description available"
                    )

                    parameters = (
                        tool_info.get("parameters", None)
                        or getattr(tool_info, "parameters", None)
                        or {}
                    )
...
if __name__ == "__main__":
    ...
    if os.environ.get("DEBUG", "false").lower() == "true":
        ...
        logger.debug(
            f"Logger level set to DEBUG, server will run on port {FASTMCP_PORT}"
        )
